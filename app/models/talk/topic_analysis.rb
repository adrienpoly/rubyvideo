class Talk::TopicAnalysis < ActiveRecord::AssociatedObject
  performs def process
    return unless talk.raw_transcript?

    response = client.chat(parameters: {model: "gpt-4o-mini", response_format:, messages:})

    content = JSON.repair(response.dig("choices", 0, "message", "content"))
    topics = begin
      JSON.parse(content).dig("topics")
    rescue
      []
    end

    talk.update! topics: Topic.create_from_list(topics)
  end

  private

  def client = OpenAI::Client.new
