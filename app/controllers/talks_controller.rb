class TalksController < ApplicationController
  ...
  def index
    ...
    if params[:q].present?
      ...
      @pagy, @talks = pagy_meilisearch(talks, limit: 9, page: session[:talks_page]&.to_i || 1)
    else
      @pagy, @talks = pagy(Talk.all.order(date: :desc).includes(:speakers, :event), limit: 9, page: session[:talks_page]&.to_i || 1)
    end
  end
